...
import { default_form_dialog } from '../site/helpers/site-defaults';
import { SmartDialogService } from 'src/app/shared/core/services/smart-dialog.service';
@Component({
  selector: 'app-smart-login-signup',
  templateUrl: './smart-login-signup.component.html',
  styleUrls: ['./smart-login-signup.component.css'],
})
export class SmartLoginSignupComponent {
  ...
  // images template
  @ViewChild('imageTemplate_1', { static: true })
  imageTemplate_1: TemplateRef<any>;
  @ViewChild('imageTemplate_2', { static: true })
  imageTemplate_2: TemplateRef<any>;
  ...
  process_login(res) {
    ...
    this.notify.logout('Logged in successfully');
    if (role == 'USER') {
  ...
  loginSubmit(data) {
    this.api.smartPost('LOG_IN', data).subscribe((res: any) => {
      let changePass =
        res['change_pass'] !== undefined ? res['change_pass'] : 0;
      if (changePass == 0) {
  ...
  resetPassword(data) {
    this.api
      .smartPost('AUTH_USER_FIRST_PASS_RESET', data)
      .subscribe((res: any) => {
